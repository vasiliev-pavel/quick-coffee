<template>
  <div>
    <nav class="navbar flex my-4 justify-between text-white font-bold text-xl rounded">
      <NuxtLink to="/" class="flex flex-col text-left p-4">
        Abu-Dhabi Mall
        <span class="text-base text-slate-400">till 11:30</span>
      </NuxtLink>
      <!-- Проверяем, залогинен ли пользователь -->
      <div v-if="userStore.name && userStore.name.user_metadata" @click="test" class="flex flex-col text-right p-4">
      ...
      <div class="flex flex-col text-right p-4" v-else>
        <button @click="toggleLoginPopup" class="text-slate-400 font-bold py-2 px-4">
          Login
        </button>
        <span class="text-base text-slate-400">350 m</span>
      </div>
